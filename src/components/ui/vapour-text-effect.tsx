...
type VaporizeTextCycleProps = {
  ...
  isHovering?: boolean; // External hover state from parent
};
...
export default function VaporizeTextCycle({
  ...
  // Combine internal and external hover states
  const effectiveIsHovering = externalIsHovering !== undefined ? externalIsHovering : isHovered;

  // Handle hover state for triggerOnHover mode (only if no external hover state provided)
  const handleMouseEnter = useCallback(() => {
    if (triggerOnHover && externalIsHovering === undefined) {
      setIsHovered(true);
    }
  }, [triggerOnHover, externalIsHovering]);
  ...
  // Track previous hover state to detect rising edge
  const prevHoveringRef = useRef(effectiveIsHovering);

  // Start animation cycle when in view (only if not triggerOnHover mode)
  useEffect(() => {
    if (triggerOnHover) {
      // Detect rising edge of hover (entering hover state)
      if (effectiveIsHovering && !prevHoveringRef.current) {
        // Start vaporizing on hover
        setAnimationState("vaporizing");
        vaporizeProgressRef.current = 0;
        resetParticles(particlesRef.current);
      }
      prevHoveringRef.current = effectiveIsHovering;
      return;
    }
  ...
  }, [isInView, triggerOnHover, effectiveIsHovering]);
      ...
      switch (animationState) {
        ...
        case "vaporizing": {
          ...
          if (vaporizeProgressRef.current >= 100 && allVaporized) {
            if (triggerOnHover) {
              // In hover mode, just fade back in without cycling text
        ...
        case "fadingIn": {
          ...
          if (fadeOpacityRef.current >= 1) {
            if (triggerOnHover) {
              // In hover mode, go back to static after fading in
              setAnimationState("static");
              resetParticles(particlesRef.current);
            } else {
  ...
  // Update wrapper style to allow pointer events when triggerOnHover is enabled
  const finalWrapperStyle = useMemo(() => ({
    ...wrapperStyle,
    pointerEvents: triggerOnHover ? ("auto" as const) : ("none" as const),
    cursor: triggerOnHover ? "pointer" : "default",
  }), [wrapperStyle, triggerOnHover]);
